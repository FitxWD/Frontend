    ...
    const fetchCurrentPlans = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/current-plans`,
          {
            headers: {
            ...
            {!isLoading && (
              <div className="grid md:grid-cols-2 gap-8 mt-10 -mb-10">
                ...
                >
                  <Link
                    href="/generate-plan"
                    className="block bg-green-500 text-white p-6 rounded-2xl shadow-2xl shadow-green-500/20 h-full"
                  >
                    <PlusIcon className="h-10 w-10 mb-4" />
                    <h3 className="text-2xl font-bold">Generate a New Plan</h3>
                    <p className="mt-2 opacity-80">
                      Chat with us to create a brand new workout and diet plan.
                    </p>
                ...
                >
                  <Link
                    href="/plan-history"
                    className="block bg-gray-800 hover:bg-gray-700/80 p-6 rounded-2xl shadow-lg h-full transition-colors"
                  >
