    ...
    <div className="grid md:grid-cols-2 gap-4 mt-6">
      {microWorkouts.map((workout) => (
        <div key={workout.name} className="bg-gray-800 p-4 rounded-lg">
          ...
          {workout.example && (
            <p className="text-sm text-gray-400 mt-2">{workout.example}</p>
          )}
          {workout.drills && (
...
export default function WorkoutsPage() {
  ...
  const assignedPlanId = searchParams.get("id");
      ...
      setTimeout(async () => {
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/workout-plan/${selectedPlan}`
          );
          if (!response.ok) {
            throw new Error(
                ...
                {planData.micro_workouts &&
                  planData.micro_workouts.length > 0 && (
                    <MicroWorkoutsSection
                      microWorkouts={planData.micro_workouts}
                    />
                  )}
                {planData.progression_4_weeks &&
                  planData.progression_4_weeks.length > 0 && (
                    <ProgressionSection
                      progression={planData.progression_4_weeks}
                    />
                  )}
                {planData.personalization_rules &&
                  planData.personalization_rules.length > 0 && (
                    <PersonalizationSection
                      rules={planData.personalization_rules}
                    />
                  )}
              </div>
            </div>
